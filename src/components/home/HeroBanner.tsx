      ...
      <div className="mx-auto px-6 z-10 relative w-full max-w-7xl">
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-y-12 md:gap-x-20 items-center min-h-[400px] transition-all duration-1000 ease-out ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          ...
          {video && video.videoUrl && (
            ...
            {/* Award badge strip */}
            <div className="mt-12 flex flex-wrap items-center justify-center md:justify-start gap-x-12 gap-y-6">
              {[ 
                { text: "2022 & 2025 NCAF&E Supporting Member of the Year", url: "https://cdn.saffire.com/files.ashx?t=fg&rid=NCAFE&f=2025_Excellence_Award_Winners(2).pdf", img: "https://storage.googleapis.com/msgsndr/d2BYZGOF7ecSj21A0t4N/media/688adfdd05df829ee4998fef.jpeg" },
                { text: "2022 Best NC Downtown Event – Electric Light Fantasy", url: "https://www.wilsonnc.org/Home/Components/News/News/475/16", img: "https://storage.googleapis.com/msgsndr/d2BYZGOF7ecSj21A0t4N/media/688adfdd05df82e3d2998fd0.png" },
                { text: "Better Business Bureau A+ rating", url: "https://www.bbb.org/us/nc/emerald-isle/profile/party-planning/emerald-owl-productions-inc-0593-90333019", img: "https://storage.googleapis.com/msgsndr/d2BYZGOF7ecSj21A0t4N/media/688adfddba7d04b5732c7d89.jpeg" }
                ...
                <a href={award.url} key={index} target="_blank" rel="noopener noreferrer" className="group flex flex-col items-center text-center transition-transform hover:scale-105">
                  {/* Badge image frame - Made much bigger */}
                  <div className="w-32 h-32 md:w-40 md:h-40 bg-white rounded-full mb-4 flex items-center justify-center group-hover:shadow-xl border-4 border-gray-200 overflow-hidden shadow-lg">
                    <Image 
                      src={award.img} 
                      alt={award.text} 
                      width={120} 
                  ...
                  <span className="text-sm md:text-base font-medium text-gray-700 group-hover:text-emerald-600 max-w-[160px] md:max-w-[200px] leading-tight">{award.text}</span>
                </a>
              ))}
            </div>
