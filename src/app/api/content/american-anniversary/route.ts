...
/**
 * GET handler for American Anniversary content
 * Reads and serves american-anniversary-celebration.json file
 */
export async function GET() {
  try {
    ...
    if (!fs.existsSync(filePath)) {
      ...
      if (!fs.existsSync(legacyFilePath)) {
        return NextResponse.json(
          { error: 'American Anniversary content not found' },
          { status: 404 }
        );
      }
    ...
    console.error('Error reading American Anniversary content:', error);
    
    // Return error response
    return NextResponse.json(
      { error: 'Failed to load American Anniversary content' },
      { status: 500 }
    );
  }
} 