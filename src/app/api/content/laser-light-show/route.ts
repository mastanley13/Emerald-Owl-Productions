...
/**
 * GET handler for laser light show content
 * Reads and serves laser-light-show.json file
 */
export async function GET() {
  try {
    ...
    if (!fs.existsSync(filePath)) {
      ...
      if (!fs.existsSync(legacyFilePath)) {
        return NextResponse.json(
          { error: 'Laser Light Show content not found' },
          { status: 404 }
        );
      }
    ...
    console.error('Error reading Laser Light Show content:', error);
    
    // Return error response
    return NextResponse.json(
      { error: 'Failed to load Laser Light Show content' },
      { status: 500 }
    );
  }
} 